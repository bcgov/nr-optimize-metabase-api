...
here::here()
#sfp_data <- read_csv(here("source", {params$data}), col_names = TRUE, col_types = #cols_only("filename" = 'c',
#			"path" = 'c', 
#			"share" = 'c', 
#			"sizemb" = col_guess(), 
#			"lastaccessdate" = col_guess(), 
#			"modificationdate" = col_guess(), 
#			))
...
formatted_sfp_data <- sfp_data %>%
  ...
  select(filename, filetype, root, path, folderdepth, sizemb, sizegb, lastaccessdate, modificationdate, monthly_cost, obj_store_cost, object_storage_cost)
```
...
table_foldersize_tallyfiles <- inner_join(calculate_folder_filetype_size,folder_filetally,by="path") %>% 
  ...
  # use this line instead if adding object storage costs
  # select(path, filetype, "number of files", "total_filetype_sizegb" = foldersizegb, monthly_cost, object_storage_cost)
```
...
# Subtract 4 because the folder depth count starts after \\KAMSFILE\S#####\
...
table_pathsize_tallyfiles <- table_pathsize_tallyfiles %>% 
  ...
  # use this line instead if adding object storage costs
  # select (path, "number of files", folderdepth, foldersizegb, foldercost, objstorcost)
```
