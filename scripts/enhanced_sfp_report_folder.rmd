...
params:
  ...
  path:
    ...
    value: "////////KAMSFILE////SHARE/////ROOT_FOLDER////SUB_FOLDER"
    input: text
  folder:
    label: "Folder Name:"
    value: "SFP_Folder"
...
sfp_data <- fread((here("source", glue("{params$data}"))), fill = TRUE, select = c("filename", "path", "share", "sizemb", "lastaccessdate", "modificationdate"), na.strings=c("","NULL.NULL")) 
```
...
formatted_sfp_data <- sfp_filter %>%
  ...
  select(filename, filetype, path, folderdepth, sizemb, sizegb, lastaccessdate, modificationdate, monthly_cost, obj_store_cost, object_storage_cost)
```
...
table_sharesize_tallyfiles <- inner_join(calculate_sharesize,share_filetype_tally,by="filetype") %>% 
  select(filetype, amount, total_size_gb = foldersizegb) %>% 
  arrange(desc(total_size_gb))
```
...
#fifthSheet = "Folder Details by Filetype"
#sixthSheet = "Filetype Details by Size"
...
#setColWidths(excel, sheet = 6, cols = c(1:3), widths = 20)
...
#addStyle(excel, sheet = 5, sty1, rows=2:(nrow(table_foldersize_tallyfiles)+1), cols=5)
