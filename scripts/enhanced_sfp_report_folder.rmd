...
params:
  data:
    ...
    input: text
  path:
    label: "Full Path to Search:"
    value: "^\\\\\\\\SERVER\\\\SHARE\\\\ROOT_FOLDER\\\\SUB_FOLDER"
    input: text
  folder:
    label: "Folder Name:"
    value: "SFP_Folder"
  ...
  collected:
    ...
    value: 2021-12-22
    input: text
title: "SFP Enhanced Report on `r params$folder`" 
...
```{r, include = FALSE}
refine_filter = paste0("\\b", params$folder, "\\b")
```

```{r folder search, include = FALSE}
search_folder <- sfp_enhanced %>%
  filter(str_detect(path, {params$path}))  %>%
  filter(str_detect(path, refine_filter)) %>% 
...
# count the number of '\'s in the path parameter, subtract 4 from beginning of path (re: escaped backslashes)
omit_depth <- stri_count_fixed(params$path, "\\") - 4
...
glue('There are {folder_count} folders in {params$folder}.')

glue('The maximum folder depth in {params$folder} is {max_depth}.')

glue('The average folder depth in {params$folder} is {mean_depth}.')
...
glue('The monthly cost of the {params$folder} folder is {share_cost}.')
...
output_excel = paste0("SFP_Enhanced_Report_", params$folder, "_", params$quarter, "_", params$fiscal, ".xlsx")
```
...
# create sheet names
firstSheet = "Path Details"
secondSheet = "Folder File Type Tally"
thirdSheet = "Share File Type Tally"
fourthSheet = "Duplicate Files"
fifthSheet = "Paths with Spaces"
sixthSheet = "Long Paths"
...
dt.worksheets(5,pathname_spaces) 
dt.worksheets(6,sfp_data_long_path)
...
setColWidths(excel, sheet = 1, cols = c(1:8), widths = c(75, 20, 20, 20, 20, 20, 20, 20))
setColWidths(excel, sheet = 2, cols = c(1:6), widths = c(75, 20, 20, 20, 20, 20))
setColWidths(excel, sheet = 3, cols = c(1:4), widths = 20)
setColWidths(excel, sheet = 4, cols = c(1, 2, 3, 4:8), widths = c(50, 20, 75, 20, 20, 20, 20, 20))
setColWidths(excel, sheet = 5, cols = 1, widths = 75)
setColWidths(excel, sheet = 6, cols = c(1, 2), widths = c(75, 20))
