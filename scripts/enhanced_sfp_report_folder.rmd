...
params:
  ...
  path:
    ...
    value: "^\\\\\\\\sfp.idir.bcgov\\\\CONTAINER\\\\SERVER\\\\SHARE\\\\ROOT_FOLDER\\\\SUB_FOLDER"
    input: text
  folder:
    label: "Folder Name:"
    value: "SFP_Folder"
...
sfp_data <- fread((here("source", glue("{params$data}"))), select = c("filename", "path", "share", "sizemb", "lastaccessdate", "modificationdate", "creationdate"))
```
...
formatted_sfp_data <- sfp_data %>%
  ...
  select(filename, filetype, root, path, folderdepth, sizemb, sizegb, lastaccessdate, modificationdate, creationdate, monthly_cost, obj_store_cost, object_storage_cost)
```
...
# count the number of '\'s in the path parameter, subtract 5 from beginning of path (re: escaped backslashes)
omit_depth <- stri_count_fixed(params$path, "\\") - 5
