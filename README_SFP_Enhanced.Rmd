...
### Gotchas
> One to look out for is when folder names are similar, but you only want to pull information from one of them (i.e. a filter on "ROB" might get you "ROB", "ROB FOI", "ROBERTSON CREEK" ). In this case, you want to also apply a *boundary* to the filter.
...
> Another that you might see is when the path has parentheses in it. You must enclose them in square brackets so that they're "escaped" from the regex expression.

**Sub-Folder search with parentheses in the pathname Example**
```{r}
#search_folder <- sfp_enhanced %>%
  #filter(str_detect(path, "^\\\\\\\\STING\\\\S40004\\\\IIT_ADMO\\\\ADMINISTRATION [(100-499)]"))
```
