...
def add_column(frame, column_ad_name, column_postgress_name):
    ...
    for i in range(len(frame.values)):
        ...
        if type(idir) is str and idir != "User ID":
...
def get_records_from_xlsx(sheet_name):
    ...
    for name in excel_names:
        ministries = ["af", "ecs", "env", "for", "irr", "mcm", "wlrs"]
    ...
    if sheet_name.lower() == "home drives":
        ...
        condition = (
            (combined["displayname"] == "NO IDIR USER RECORD FOUND") |
            (combined["mailboxcode"].isna()) |
            (combined["mailboxcode"] == "")
        )

        # Update Mailbox Org Code based on ministry
        combined.loc[condition, "mailboxcode"] = combined.loc[condition, "ministry"]
        ...
        combined.drop(combined[combined.idir == "NCOSTEMP"].index, inplace=True)
