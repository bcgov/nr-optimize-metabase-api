...
table_sharesize_tallyfiles <- inner_join(calculate_sharesize,share_category_tally,by="category") %>% 
  select(category, amount, foldersizegb) %>% 
...
table_foldersize_tallyfiles <- inner_join(calculate_folder_category_size,folder_filetally,by="path") %>% 
  select(path, category, "number of files", "total_category_sizegb" = foldersizegb, monthly_cost)
...
glue('Dupicate files were identified in the {params$share} share that total {dup_cost} per month.') 

glue('Removing the duplicates in {params$share} could save at least {dup_cost_half} per month!')
```
